# Automatic parcel tracking from e-mail (IMAP → Budbee)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the Budbee tracking code, and register it with `budbee.track_parcel` — fully automatic, no extra custom component required.

Budbee is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `budbee.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ budbee.track_parcel
                                     │
...
- This integration, with the `budbee.track_parcel` action available (field `tracking_code`).
...
The regex matches the **tracking link**, not the number:

```
(?:track|tracking)[.](?:budbee|instabox|porterbuddy)[.]com/([0-9A-Za-z]{4,40})
```

Budbee numbers carry no prefix and no fixed length — the one confirmed sample is 12 uppercase alphanumerics, and the integration itself accepts anything matching `^[A-Z0-9]{4,40}$`. Matching *that* against e-mail text would pick up every order, invoice and customer number in the mail, so the example anchors on the link instead: shipping confirmations always contain one, and the number is its last path segment.

Three hosts are covered. `track.budbee.com` is the current one, `tracking.budbee.com` the older one still in use, and `instabox.com` / `porterbuddy.com` are Budbee's sibling brands, which run on the same backend — a link to either resolves the same parcel.
...
1. **Jinja eats backslashes in string literals.** A template stored as `regex_findall('\btrack.budbee.com…')` silently becomes a **backspace character** (`\b` is a string escape), so the regex never matches — no error anywhere. That's why the pattern is backslash-free: `(?<![0-9A-Za-z])` lookarounds instead of `\b`, `[0-9]` instead of `\d`. Copy that style if you extend the pattern.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@budbeeshipping.com","subject":"Your parcel is on its way",
       "text":"Track your parcel: https://track.budbee.com/BUDBEE123456","initial":true,"folder":"INBOX","username":"test"}'
```

Then `budbee.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
