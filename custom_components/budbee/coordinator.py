...
def _hottest_tier_minutes(active_parcels: list[dict], now: datetime) -> int | None:
    ...
    filtered out of ``active_parcels`` by the caller).
    """
...
class BudbeeCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        if setting == REFRESH_INTERVAL_AUTO:
            ...
            # Section 6): a parcel the user is sending out for delivery is
            # just as "happening in the next hour" as one arriving.
