"""Calendar platform for the Budbee parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the Budbee deliveries calendar from a config entry."""
    async_add_entities([BudbeeDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class BudbeeDeliveriesCalendar(CoordinatorEntity[BudbeeCoordinator], CalendarEntity):
    """A read-only calendar of expected Budbee deliveries.
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "Budbee parcel")
            description_parts = [
