"""Services for the Budbee parcel tracker integration.

`budbee.track_parcel` / `budbee.untrack_parcel` let you add or remove a
...
def _resolve_entry(hass: HomeAssistant):
    """Return the single Budbee hub, or raise when it is not set up."""
    entries = hass.config_entries.async_entries(DOMAIN)
    if not entries:
        raise ServiceValidationError("Budbee is not set up")
    return entries[0]


def async_setup_services(hass: HomeAssistant) -> None:
    """Register the Budbee services (idempotent)."""
    ...
    async def _track(call: ServiceCall) -> None:
        ...
        if not valid_tracking_code(tracking_code):
            raise ServiceValidationError(
                f"'{tracking_code}' is not a valid Budbee tracking code"
            )
...
def async_unload_services(hass: HomeAssistant) -> None:
    """Remove the Budbee services (single-entry integration, so on unload)."""
