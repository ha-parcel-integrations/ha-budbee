"""pytest configuration for the Budbee test suite."""
import sys
...
def auto_enable_custom_integrations(enable_custom_integrations):
    """Make ``custom_components.budbee`` loadable from config-flow / setup tests."""
    yield


@pytest.fixture(autouse=True)
def reset_one_shot_warnings():
    """Clear the "already reported this session" sets between tests.

    Both the API client and the parcel mapping log each unknown exactly once
    per HA session. Those sets are module-level, so without this a test's
    warning assertion would depend on which test ran first.
    """
    from custom_components.budbee import api, parcels
