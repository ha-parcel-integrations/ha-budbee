"""Tests for Budbee setup and unload."""
...
from .payloads import delivery_sample as _sample

OTHER_CODE = "BUDBEE222222"
...
async def test_setup_and_unload(hass):
    ...
    with patch(
        "custom_components.budbee.api.BudbeeApiClient.async_get_parcel",
        new=AsyncMock(return_value=_sample(ACTIVE_CODE)),
    ):
    ...
    incoming = hass.states.get("sensor.budbee_incoming_parcels")
...
async def test_setup_retries_when_first_refresh_fails(hass):
    ...
    with patch(
        "custom_components.budbee.api.BudbeeApiClient.async_get_parcel",
        new=AsyncMock(side_effect=BudbeeApiError("Budbee unreachable")),
    ):
...
async def test_per_parcel_sensor_spawn_and_remove(hass):
    ...
    mock = AsyncMock(return_value=_sample(ACTIVE_CODE))
    with patch("custom_components.budbee.api.BudbeeApiClient.async_get_parcel", new=mock):
        assert await hass.config_entries.async_setup(entry.entry_id)
        await hass.async_block_till_done()
