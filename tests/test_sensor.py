"""Tests for Budbee sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.budbee.const import ParcelStatus
from custom_components.budbee.sensor import (
    BudbeeAwaitingPickupSensor,
    BudbeeDeliveredParcelsSensor,
    BudbeeIncomingParcelsSensor,
    BudbeeLastUpdateSensor,
    BudbeeNextDeliverySensor,
    BudbeeParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = BudbeeIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_awaiting_pickup_counts_only_ready_pickup_point_parcels():
    ...
    sensor = BudbeeAwaitingPickupSensor(
...
def test_awaiting_pickup_zero_when_no_parcels():
    sensor = BudbeeAwaitingPickupSensor(_coordinator([]), _entry())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = BudbeeParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = BudbeeParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = BudbeeNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = BudbeeNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = BudbeeDeliveredParcelsSensor(coordinator, _entry())
    assert sensor.native_value == 1
...
def test_last_update_sensor():
    ...
    sensor = BudbeeLastUpdateSensor(coordinator, _entry())
