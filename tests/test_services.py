"""Tests for the Budbee services (track_parcel / untrack_parcel)."""
...
from .payloads import delivery_sample

_SAMPLE = delivery_sample()
...
async def test_track_parcel_adds_to_options(hass):
    ...
    with patch(
        ...
        await hass.services.async_call(
            DOMAIN,
            "track_parcel",
            {CONF_TRACKING_CODE: "BUDBEE999999"},
            blocking=True,
        )
        await hass.async_block_till_done()

    parcels = entry.options[CONF_PARCELS]
    assert parcels == [{CONF_TRACKING_CODE: "BUDBEE999999"}]
...
async def test_track_parcel_normalizes_code(hass):
    ...
    with patch(
        ...
        await hass.services.async_call(
            DOMAIN,
            "track_parcel",
            {CONF_TRACKING_CODE: "budbee-999 999"},
            blocking=True,
        )
        await hass.async_block_till_done()
...
async def test_untrack_parcel_removes_from_options(hass):
    entry = await _setup(
        hass, parcels=[{CONF_TRACKING_CODE: "BUDBEE999999"}]
    )
    with patch(
        "custom_components.budbee.api.BudbeeApiClient.async_get_parcel",
        new=AsyncMock(return_value=_SAMPLE),
...
async def test_untrack_unknown_code_is_noop(hass):
    ...
    with patch(
        ...
        await hass.services.async_call(
            DOMAIN,
            "untrack_parcel",
            {CONF_TRACKING_CODE: "BUDBEE000000"},
            blocking=True,
        )
        await hass.async_block_till_done()
